...
FILE_LABELS_PATH = "../../Datasets/weak_supervision_results.csv"
...
count_labels = {i:0 for i in range(4)}
test_file = []
for label, file_name, pred in file_labels:
    ...
    print(label_left)
    if(count_labels[label] < 15 and count_labels[label_left] < 15):
        if(label_left == label and count_labels[label] == 14):
            continue
