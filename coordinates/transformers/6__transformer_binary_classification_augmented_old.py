...
train_data["labels"] = (np.array(train_data["labels"]) != 0).astype(float)
val_data["labels"]   = (np.array(val_data["labels"])   != 0).astype(float)
test_data["labels"]  = (np.array(test_data["labels"])  != 0).astype(float)
...
# Convert labels: non-zero becomes 1, zero remains 0
# train_data["labels"] = (np.array(train_data["labels"]) != 0).astype(float)
# val_data["labels"]   = (np.array(val_data["labels"])   != 0).astype(float)
# test_data["labels"]  = (np.array(test_data["labels"])  != 0).astype(float)
...
# -------------------------------
temp_train_dataset = VideoDataset(train_data_aug["file_names"], train_data_aug["labels"], train_data_aug["coordinates"],
                                  max_frames=max_frames, transform=None)
...
# -------------------------------
train_dataset = VideoDataset(train_data_aug["file_names"], train_data_aug["labels"], train_data_aug["coordinates"],
...
train_labels = np.array(train_data_aug["labels"]).astype(int)
# Count occurrences of each class (assuming classes 0 and 1)
class_counts = np.bincount(train_labels)
total_samples = len(train_labels)
# Calculate weights: weight[i] = total_samples / (num_classes * count[i])
class_weights = total_samples / (num_classes * class_counts)
print("Class counts:", class_counts)
print("Computed class weights:", class_weights)

# -------------------------------
# 6. Model Building: Advanced Transformer-Based Classifier
# -------------------------------
...
class TransformerClassifier(nn.Module):
    def __init__(self, input_dim=63, d_model=48, nhead=4, num_layers=4, ff_dim=128, dropout=0.2, seq_length=max_frames, num_classes=2):
        ...
        self.fc4 = nn.Linear(32, num_classes)   # Output layer: logits for 2 classes
        self.relu = nn.ReLU()
...
# Use computed class weights in the loss function
...
# -------------------------------
# 10. Plotting Learning Curve and Confusion Matrices
# -------------------------------
